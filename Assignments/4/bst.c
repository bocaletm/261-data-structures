...
{
    struct Node* temp; 
        //base case; empty node is reached
    if (cur == 0) {
        ...
        return temp;
            ...
            //move left and recursively call this function
        cur->left = _addNode(cur->left,val);
    } else {
            //move right and recursive call
        cur->right = _addNode(cur->right,val);
    }
        //rebuild tree on way back
    return cur;
}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    struct Node* iter = tree->root;
        //traverse the tree while value is not found
    while (compare(iter->val,val) != 0) {
        if (compare(val,iter->val) == -1) {
                //if we hit a null node, val not in tree
            if (iter->left == 0) {
                return 0;
            } else {
                iter = iter->left;
